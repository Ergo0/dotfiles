...
__webpack_require__(9); // eslint-disable-line
...
const OptionsPanel = __webpack_require__(5); // eslint-disable-line

__webpack_require__(51); // eslint-disable-line
...
class RabbitPopup {
	...
	initalizePopup() {
		...
		chrome.tabs.query({ active: true, currentWindow: true }, async activeTabs => {
			...
			await this._extensionController.getMySpaces().then(spaces => {
				this._spaces = spaces;
			});
	...
	showStartPanel() {
		...
		errorPanel.style.display = activeRooms.length === 0 ? 'inline-block' : 'none';
		...
		if (activeRooms.length === 1) {
			...
			// show i am sad and need to be in a room
			startPanel.style.display = 'none';
			roomsPanel.style.display = 'none';
			errorPanel.style.display = 'inline-block';
		} else {
...
const RabbitPopup = __webpack_require__(16); // eslint-disable-line import/no-unresolved
